...
namespace GestionBasica.GUI
{
    public partial class Permisos : Form
    {
        ...
        private void dtgPermisos_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (MessageBox.Show("ESTA SEGURO QUE DESEA EDITAR EL PERMISO?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (e.ColumnIndex == 0)
                    {
            ...
            {
                MessageBox.Show("Ocurrio un Error a la hora de modificar el permiso", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
