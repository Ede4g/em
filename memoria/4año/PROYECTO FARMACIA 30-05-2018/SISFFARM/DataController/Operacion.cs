...
namespace DataController
{
    public class Operacion:Conexion
    {
        ...
        private Int32 EjecutarSentenciaConImagen(String pSentencia, byte[] pImagen)
        {
            Int32 FilasAfectadas = 0;
            try
            {
                if (base.Conectar())
                {
                    MySqlCommand Comando = new MySqlCommand();
                    Comando.CommandText = pSentencia;
                    Comando.Parameters.AddWithValue("Imagen", pImagen);
                    Comando.Connection = base._CONEXION;
                    FilasAfectadas = Comando.ExecuteNonQuery();
                }
            }
        ...
        ///
        public Int32 Insertar(String pNombre, byte[] pImagen)
        {
            return EjecutarSentenciaConImagen(pNombre, pImagen);
        }
        public Int32 Actualizar(String pNombre, byte[] pImagen)
        {
            return EjecutarSentenciaConImagen(pNombre, pImagen);
        }
    }
